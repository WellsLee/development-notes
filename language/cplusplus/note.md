...
* 转换函数采用如下的一般形式：operator type(); type可用内置类型，类类型或者typedef名取代，但不允许type表示函数或数组，转换函数必须是成员函数，它的声明不能指定返回类型和参数表。

# 构造函数
* 构造函数一般为public，如不允许创建对象则为private，如单例。
* 默认构造函数（缺省构造函数）：指为所有参数都提供了默认值的构造函数,通常是指无参的构造函数，这并不意味它不能接受实参。
* 子类不会继承父类的构造函数和析构函数。
...
* explicit修饰符通知编译器不要提供隐式转换，explicit只能应用在构造函数上。static_cast可以以强制转换的形式显式地要求转换。
...
# 派生类
* 每个派生类的公有接口都是由基类的公有成员和派生类自己的公有成员构成。
* 为了用已被派生类重用的名字来访问基类的成员，必须用它的类域操作限定符修饰基类的成员。常见错误：希望基类和派生类的成员函数构成一个重载的函数集。
* 派生类不能访问另一个独立的基类对象的protected成员。
* 基类指针只能访问在该类中被声明或继承的数据成员和成员函数，包括虚拟成员函数，而与它指向的实际对象无关。
* 如果一个派生类希望直接访问其基类的私有成员，则该基类必须显式地把派生类声明为友元。
* 为了向基类构造函数传递一个或多个参数，使用成员初始化表。
* 构造函数的调用顺序如下：1）基类的构造函数；2）成员类对象的构造函数；3）派生类的构造函数。
* 派生类不继承基类的构造函数。
* 当派生类对象的生命周期结束时，派生类，基类，成员类对象的析构函数被自动调用，调用的顺序跟构造函数相反。
* public派生类是类型继承；private派生类是实现继承，基类的公有成员在派生类中变成private；在protected继承下，基类的所有公有成员都成为派生类的protected成员。
* 对于支持is-a子类型关系来说，继承是一个很有力的机制；has-a关系一般是由组合而不是继承来支持。
* 一种常见的多继承形式是，继承一个类的公有成员和第二个类的私有实现。
* 有两种形式的对象组合：1）按值组合，类的实际对象被声明为一个成员；2）按引用组合，通过类对象的引用或指针成员间接指向一个对象。
* 在继承下，派生类的域被嵌套在基类的域中。

# 虚函数
* 非虚函数的调用在编译时刻选择被调用的函数，而虚函数要等到运行时刻。
* 虚函数机制只在使用指针和引用时才会起作用。
* 第一次引入虚拟函数时，必须在类声明中指定virtual关键字，如果定义被放在类的外面，则不能再次指定关键字virtual。引入虚函数的类必须定义它或者把它声明为纯虚函数。
* 派生类的成员函数的返回值可以是基类成员函数返回类型的公有派生类型。
* 包含（或继承）一个或者多个纯虚函数的类被编译器识别为抽象基类，不能创建抽象基类的实例对象。
* 虚拟函数的静态调用：当有类域操作符调用虚函数时，我们改变了虚拟机制，使虚拟函数在编译时刻被静态解析。
* 虚函数缺省实参：缺省实参不是运行时刻决定的，而是在编译时刻根据对象的类型决定的。
* 作为一般原则，我们建议将抽象基类的析构函数声明为虚拟的，一般地，基类的析构函数不应该是protected。
* 在基类构造函数中调用的虚拟实例总是在基类中活动的虚拟实例。

# 多态
* 通过下面的方式支持多态：1）通过隐式转换，从派生类指针或引用转换到其公有基类的指针或引用。2）通过虚函数。3）通过dynamic_cast和typeid操作符。
* 派生类的前向声明不能包含它的派生类。class NameQuery : public Query; //错误
* 抽象基类只能定义它的指针和引用，被用来间接操纵从它派生出来的子类。

# RTTI(运行时刻类型识别)
* dynamic_cast操作符：在运行时刻进行类型转换，把基类指针转换成派生类指针，或把指向基类的左值转换成派生类的引用。
* typeid操作符：它指向指针或引用指向对象的实际派生类型。
* 对于要获得派生类类型的信息，dynamic_cast和typeid操作的类型必须是带有一个或多个虚拟函数的类类型。应该尽量减少使用RTTI。
